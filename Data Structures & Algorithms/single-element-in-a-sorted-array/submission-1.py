class Solution:
    def singleNonDuplicate(self, nums: List[int]) -> int:
        ...
        # if mid is even, we check if the element right of mid == mid. if so then RS has single element else LS does. 
        #  if mid is odd, that means we need to check its left val (which is supposed to be its pair), if == then RS has single element, else LS.
